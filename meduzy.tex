...
\captionof{figure}{Graf $\AG_{\mathbb{F}_7}$.}
...
\caption{Neisomorfní komponenty souvislosti grafu $\AG_{\mathbb{F}_{11}}$.}
\end{figure}
...
\begin{figure}[h]
  \includegraphics[width=15cm]{SCountLOL.png}
  \label{fig:boat1}
...
Podívejme se na graf $\AH_{\mathbb{F}_{11}}$ a dva prvky, $(1,3)$ a $(3,2)$. Komponenty souvislosti obsahující tyto dva prvky vypadají následovně:\\
...
\node [draw,black,fill=yellow, circle,inner sep=\t, scale=\s, name=b2]      at (0:3cm) {$2,3$};
...
\node [draw,black,fill=yellow, circle,inner sep=\t, scale=\s, name=d3]      at (20:5cm) {$5,10$};
\node [draw,black,fill=yellow, circle,inner sep=\t, scale=\s, name=d4]      at (340:5cm) {$10,5$};
...
Dále se podívejme na graf $\AH_{\mathbb{F}_{23}}$. 
\end{priklad}
...
Kdy nastává který případ? Lze ukázat [?], že pro křivku $E \in \Ell(\mathbb{F}_q)$ splňující $\End(E) = \mathbb{Z}[\pi]$ existuje jediná isogenie $\phi : E \longrightarrow E^{\prime}$ stupně dva a navíc platí $\End(E^{\prime}) = \mathbb{Z}\left[\frac{1+\pi}{2}\right]$. Z křivky $E^{\prime}$ vychází tři isogenie stupně $2$ definované nad $\mathbb{F}_q$. Duální isogenie $\widehat{\phi} : E^{\prime} \longrightarrow E$ zobrazí křivku $E^{\prime}$ na křivku s \uv{menším} okruhem endomorfismů, zbylé dvě isogenie zobrazí $E^{\prime}$ na křivky, jejichž okruh endomorfismů je $E^{\prime \prime} \cong  \mathbb{Z}\left[\frac{1+\pi}{2}\right]$.
