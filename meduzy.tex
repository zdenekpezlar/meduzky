...
\thispagestyle{empty}
\subsection*{Abstrakt}
Cílem práce je studovat posloupnosti průměru nad konečnými tělesy. Tzv. AG posloupnost studovaná už Gaussem má překvapivá propojení s eliptickými integrály a lze ji využít na efektivní výpočty elementárních funkcí. Nejprve studujeme adaptaci této posloupnosti nad konečnými tělesy a ukážeme, že má propojení s vulkány eliptických křivek a grupou tříd ideálů jistých okruhů. Hlavní přínos práce spočívá ve formulaci a studiu adaptaci AH posloupnosti nad konečným tělesem. V závěru práce tuto posloupnost spojíme s jistou singulární eliptickou křivku a díky tomu plně určíme tvary, kterých AH posloupnost nabývá, a studujeme jejich vlastnosti. 


\subsection*{Klíčová slova}
Aritmeticko-geometrický průměr, Aritmeticko-harmonický průměr, eliptické křivky, isogenie, grupa tříd ideálů
...
Arithmetic-Geometric Mean, Arithmetic-Harmonic Mean, Elliptic Curves, Isogeny, Ideal Class Group
...
\end{equation*}
Platí tedy $a_n \geqslant b_n$ pro kladná $n$. Pro $n$ jdoucí k nekonečnu čísla $a_n$ i $b_n$ konvergují ke společné limitě, tzv. \textit{aritmetickému geometrickému průměru} čísel $a,b$. Tento průměr studoval již Carl Friedrich Gauss a ukázal, že tato na první pohled nevinná posloupnost je spojena s eliptickými integrály. Díky tomuto propojení lze posloupnost využít k rychlému počítání čísla $\pi$ i evaluování funkcí jako $e^x$ či $\arcsin(x)$. 

Co se ale na posloupnost podívat v jiném světle, konkrétně nad konečnými tělesy? V~jistých tělesech můžeme jednoznačně adaptovat naši posloupnost. Tentokrát posloupnost již ne vždy nekonverguje, zato však tvoří možná zajímavější struktury. Pokud sestavíme orientované grafy popisující naši posloupnost pro všechny dvojice $(a,b)$ nad naším tělesem, získáme grafy, které vypadají následovně:\\
...
\end{center}
\end{figure}
Tento graf nazveme \textit{medúzou}. Už to, že grafy tvoří takovéto struktury je pozoruhodné, medúzy ale zde zdaleka nekončí. Ukážeme, že tyto medúzy jsou spojené s grafy eliptických křivek a setkáme se s tzv. \textit{grupou tříd ideálů}. Výpočetně ověříme jisté odhady položené v \cite{Meduza} a podíváme se podrobně na hodnoty spojené s těmito grafy, nejpodstatněji na počet komponent souvislosti v tomto grafu.
...
Zatím jsme pracovali s dvěma dvojicemi průměrů z trojice -- aritmetický, geometrický a~harmonický. V této kapitole se proto podíváme i na tu poslední -- aritmetický a harmonický průměr.
...
Tentokrát již ze začátku nebude pracovat pouze nad konečným tělesem, ale i s bodem v nekonečnu.
...
Dále se podívejme na graf $\AH_{\mathbb{F}_{23}}$. Komponenta souvislosti obsahující prvek $(1,5) \in \AH_{\mathbb{F}_q}$ vypadá následovně.
...
Kdy nastává který případ? Lze ukázat [?], že pro křivku $E \in \Ell(\mathbb{F}_q)$ splňující $\End(E) = \mathbb{Z}[\pi]$ existuje jediná isogenie $\phi : E \longrightarrow E^{\prime}$ stupně dva a navíc platí $\End(E^{\prime}) = \mathbb{Z}\left[\frac{1+\pi}{2}\right]$. Z křivky $E^{\prime}$ vychází tři isogenie stupně $2$ definované nad $\mathbb{F}_q$. Duální isogenie $\widehat{\phi} : E^{\prime} \longrightarrow E$ zobrazí křivku $E^{\prime}$ na křivku s \uv{menším} okruhem endomorfismů, zbylé dvě isogenie zobrazí $E^{\prime}$ na křivky, jejichž okruh endomorfismů je $\End(E^{\prime \prime}) \cong  \mathbb{Z}\left[\frac{1+\pi}{2}\right]$. 
