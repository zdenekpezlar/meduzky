...
\item $\AG(1-x,1+x) = \AG(a,b)$, kde $x = \frac{1}{a} \sqrt{a^2 - b^2}$.\footnote{hmmmmmm}
\end{enumerate}
\end{veta}
...
\begin{definice}
Souvislý orientovaný graf $G$ nazveme \textit{medúzou}, pokud je tvořen jediným cyklem $H$ a navíc pro každý vrchol $V \in H$ existuje unikátní předchůdce\footnotemark FOOTNOTE FOOTNOTE FOOTNOTE mimo cyklus, který sám nemá předchůdce.
\end{definice}
...
\end{align*}
\end{definice}
Obě tato čísla jsou v algebře endomorfismů celá čísla. Navíc je jednoduché ukázat, že stopa je lineární zobrazení na $\End^0 (E)$ a norma je multiplikativní. Pomocí normy a stopy pak můžeme stanovit následující věty.
\begin{veta}
Buď $\alpha \in \End^0 (E)$ nenulové. Potom existuje prvek $\alpha^{-1} \in \End^0 (E)$ splňující $\alpha \alpha^{-1} = 1 = \alpha^{-1} \alpha$. 
\end{veta}
\begin{veta}
Buďte $\alpha \in \End^0 (E)$ a $r \in \mathbb{Q}$. Pak platí $r \alpha = \alpha r$ a $\pi \alpha = \alpha \pi$.
\end{veta}
O algebře endomorfismů a okruhu endomorfismů Legendreho křivky pak můžeme říct následující.
\begin{dusledek}
Buď $E$ Legendreho křivka. Pak platí $\End^0(E) \cong \mathbb{Q}(\pi)$. Okruh $\End (E)$ je volným $\mathbb{Z}$-modulem v $\End^0 (E)$ ranku $2$.
\end{dusledek}
Jistě platí inkluze okruhů $\mathbb{Z}[\pi] \subseteq \End(E)$. Navíc v kvadratickém $K$ tělese je každý volný $\mathbb{Z}$-modul obsažen v okruhu celých algebraických čísel $\mathcal{O}_K$. Okruh celých algebraických čísel je obsažen v okruhu $\mathbb{Z}\left[\frac{1+\pi}{2}\right]$, platí proto $[\End(E):\mathbb{Z}[\pi]] \in \lbrace 1,2 \rbrace$.
...
\end{veta}
\noindent \textit{Důkaz.} Využijeme větu \ref{BRUH}. Je známé [IR], že grupa $\mathbb{F}_q ^{\times}$ je cyklická, tj. existuje prvek $g \in \mathbb{F}_q ^{\times}$, který ji generuje. To znamená, že ???? 
